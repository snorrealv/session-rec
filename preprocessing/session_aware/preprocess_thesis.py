...
SESSION_THRESHOLD = 200 * 60
# filtering config (all methods)
MINIMUM_OCCURRENCES_PER_ITEM = 5
...
def prepare_time(data, time_key=TIME_KEY):
    ...
    data[time_key] = pd.to_datetime(data[time_key]).astype(int) / 10**9
...
def filter_data(data, min_item_support=MINIMUM_OCCURRENCES_PER_ITEM, min_session_length= MIN_SESSION_LENGTH, min_user_sessions=MIN_USER_SESSIONS, max_user_sessions=MAX_USER_SESSIONS):
    ...
    # condition = data.groupby(USER_KEY)[SESSION_KEY].nunique().min() >= min_ and \
    #         data.groupby([USER_KEY, SESSION_KEY]).size().min() >= MINIMUM_RECORDS_PER_SESSION and \
    #         data.groupby([ITEM_KEY]).size().min() >= MINIMUM_OCCURRENCES_PER_ITEM
